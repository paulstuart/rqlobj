		...
		if t := field.Tag; t != nil {
			...
			if sql := tag.Get("sql"); len(sql) > 0 {
				...
				if key := tag.Get("key"); len(key) > 0 {
					...
					info.KeyField = sql
					info.NoUpdate[info.KeyName] = struct{}{}
				} else {
...
func (g *Generator) buildWrappers(s *SQLInfo) {
	...
	g.Printf(stringSQLResults, s.Name, s.Table)
	...
	g.Printf(stringInsertFields, s.Name, strings.Join(insert_fields, ","))
